...
#pragma clang diagnostic pop
    ...
    fadeRange = 1.0;//[self valueForOption:iCarouselOptionFadeRange withDefault:fadeRange];
...
//        if(offset == 1.0)
//            return 1.0f - fminf(offset - fadeMax, fadeRange) / fadeRange + 0.5;
...
{   
    ...
    switch (_type)
    {
        ...
        {
            CGFloat count = 5.0;//[self circularCarouselItemCount];
            CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:0.68f];
            CGFloat arc = [self valueForOption:iCarouselOptionArc withDefault:M_PI * 0.2f];
            CGFloat radius = [self valueForOption:iCarouselOptionRadius withDefault:fmaxf(0.01f, _itemWidth * spacing / 1.5f / tanf(arc/2.5f/count))];
            CGFloat angle = [self valueForOption:iCarouselOptionAngle withDefault:offset / count * arc];
            ...
            {
                transform = CATransform3DTranslate(transform, 0.0f, 3.0f, -radius);
                transform = CATransform3DRotate(transform, angle, 0.0f, 1.0f, 0.0f);
                return CATransform3DTranslate(transform, 0.0f, 0.0f, radius + 3.0f);
            }
        }
...
- (void)updateNumberOfVisibleItems
{
    ...
    switch (_type)
    {
        ...
        case iCarouselTypeInvertedWheel:
        {
            //TODO: improve this
            CGFloat count = [self circularCarouselItemCount];
            CGFloat spacing = [self valueForOption:iCarouselOptionSpacing withDefault:2.0f];
            CGFloat arc = [self valueForOption:iCarouselOptionArc withDefault:M_PI * 2.0f];
            CGFloat radius = [self valueForOption:iCarouselOptionRadius withDefault:_itemWidth * spacing * count / arc];
